%% HACK for Tralics, 
\begin{filecontents+}{amsldoc.clt}
...
\ProcessOptions
\let\allowtthyphens \relax
\def\chaptername{Chapter}
\def\pkg#1{\xbox{EM}{\AddAttToCurrent{r}{pkg}#1}}
\def\opt#1{\xbox{EM}{\AddAttToCurrent{r}{opt}#1}}
\def\cn#1{\xbox{EM}{\AddAttToCurrent{r}{cn}#1}}
\def\ncn#1{\xbox{EM}{\AddAttToCurrent{r}{ncn}#1}}
\def\cs#1{\xbox{EM}{\AddAttToCurrent{r}{cs}#1}}
\def\indexcs#1{\xbox{EM}{\AddAttToCurrent{r}{indexcs}#1}}
\def\fn#1{\xbox{EM}{\AddAttToCurrent{r}{fn}#1}}
\def\cls#1{\xbox{EM}{\AddAttToCurrent{r}{cls}#1}}
\def\env#1{\xbox{EM}{\AddAttToCurrent{r}{cls}#1}}
...
\def\tex/{\TeX}
\def\latex/{\LaTeX}
\def\amslatex/{\xbox{amslatex}{}}
\def\amstex/{\xbox{amstex}{}}
\def\xypic/{\xbox{xypic}{}}
\let\trivlist\relax \let\endtrivlist\relax
...
\let\error\relax
\newenvironment{error}{\tracingall%
  \begingroup\catcode`\\=12 \catcode`\&=12  \expandafter\endgroup\errora
}{%
\end{xmlelement*}
}
...
list of the \cn{usepackage}\verb|{amsmath}| statement.
\begin{description}
...
particular line by putting \cn{notag} before the \cn{\bslchar\bslchar}; you can also
override\index{equation numbers!overriding} it with a tag of your own
using \cn{tag}\verb|{|\<label>\verb|}|, where \<label> means arbitrary text such
as \verb|$*$| or \verb|ii| used to \qq{number} the equation. There is also a
...
line as an argument, up to but not including the final \cn{\bslchar\bslchar}; for
example
...
environment provides for alignment among the split lines, using \verb|&| to
...
Equations inside \env{gather} are separated by a \verb|\\| command.
...
\verb|\raisetag{6pt}|. This kind of adjustment is fine tuning like line
...
You can use the \verb|\\[|\<dimension>\verb|]| command to get extra vertical
...
optional argument 1\ndash 4 can be used for finer control: \verb|[1]| means
...
\cn{allowdisplaybreaks}, the \verb|\\*| command can be used to prohibit a
...
\verb|\\| or \verb|\\*| command. Notice the position of the word \qq{and} in
this example.
\begin{align}
...
by the counter \cn{MaxMatrixCols} (normal value = 10), which you can change
...
matrix. Delimiters must be provided; there are no \verb|p|,\verb|b|,\verb|B|,\verb|v|,\verb|V|
...
\cn{hdotsfor}\verb|{|\<number>\verb|}| produces a row of dots in a
...
get a negative \cn{quad} you could write \verb|\mspace{-18.0mu}|.
...
good: $\sqrt[\beta]{k}$ (\verb|\sqrt|\verb|[\beta]{k}|).  In the
...
else. The input \verb|\overset{*}{X}| will place a superscript-size $*$ above
...
convenient abbreviations for \verb|{\displaystyle\frac| \verb|...| \verb|}|
and\indexcs{textstyle}\relax
\indexcs{displaystyle} \verb|{\textstyle\frac| \verb|...| \verb|}|.
\begin{equation}
...
arguments \verb|t| and \verb|b|, because occasionally it is advantageous to be
...
\verb"$\sqrt{x}" \verb"+"
\verb"\sqrt{"+\verb"\smash[b]{y}}" \verb"+" \verb"\sqrt{z}$".
...
larger than necessary, and using the \verb|Big| or \verb|bigg|
...
problem of overloading for the vert bar character \cn{|}. This
...
\cn{|} and a selected mathematical notation, and similarly for the
double-bar command \ncn{|}\index{this-is-wrong}. This immediately
rules out the use of \cn{|}
and \ncn{|}\index{this-is-wrong} for delimiters, because left and right
...
\providecommand{\norm}[1]{\lVert#1\rVert}
\end{verbatim}
whereupon the document would contain \verb|\abs{z}| to produce $\lvert
...
the hyphen character \cn{-} will print as a text hyphen rather than a
minus sign and an asterisk \cn{*} will print as a raised text asterisk
...
the \cn{*} form of the \cn{DeclareMathOperator} command:
\begin{verbatim}
...
subscript, you can type \verb|..._{\text{word or phrase}}|, which is slightly
easier than the \cn{mbox} equivalent: \verb|..._{\mbox{\scriptsize| \verb|word|
\verb|or| \verb|phrase}}|. The other advantage is the more descriptive name.
\begin{equation}
...
\end{CD}
\end{verbatim}
In the \env{CD} environment the commands \verb|@>>>|,
\verb|@<<<|, \verb|@VVV|, and \verb|@AAA| give respectively right, left, down, and up
arrows. For the horizontal arrows, material between the first and second
\verb|>| or \verb|<| symbols will be typeset as a superscript, and material
between the second and third will be typeset as a subscript. Similarly,
material between the first and second or second and third \verb|A|s or \verb|V|s
of vertical arrows will be typeset as left or right \qq{sidescripts}.
The commands \verb|@=| and \verb'@|' give horizontal and vertical double lines.
A \qq{null arrow} command \verb|@.| can be used instead of a visible arrow
...
\begin{verbatim}
  \newcommand{\vect}[1]{\mathbf{#1}}
\end{verbatim}
you can write \verb|\vect{a} + \vect{b}| to produce $\vect{a} +
\vect{b}$.
...
\ncn{field} command and writing\break \verb|\field{C}|, \verb|\field{R}|. But for
...
unimportant text such as \verb|! LaTeX Error:| at the beginning, and
nonalphabetical characters such as \verb|\\|. Where examples are given, we
show also the help messages that appear on screen when you respond to an
error message prompt by entering \verb|h|.
...
is dictated by the numeric argument given after \verb|\begin{alignat}|.
If you use more alignment points in a line it is assumed that you
accidentally left out a newline command \verb|\\| and the above error is
issued.
\end{error}
...
\end{verbatim}
\errexpl
This usually follows a previous error \verb|Font ... not loadable|; see the
...
Another possibility is that you have a left bracket character \verb|[|
following a linebreak command \verb|\\| in a multiline construction such
...
\end{verbatim}
\errexpl
This might be produced by a misspelling in the \verb|\end{multline}| command,
e.g.,
\begin{verbatim}
\begin{multline}
...
\end{multiline}
\end{verbatim}
or by using abbreviations for certain environments, such as \verb|\bal| and
\verb|\eal| for \verb|\begin{align}| and \verb|\end{align}|:
\begin{verbatim}
\bal
...
\eal
